...
::: {.column style="font-size:15px"}
Es el momento de introducir otro modelo, como lo es la regresión logistica, a paser de su nombre, la regresión logistica se utiliza para la clasificación. Este modelo, calcula la probabilidad de que una observación pertenezca a una clase binaria. Utilizando nuestro conjunto de datos sobre la diabetes como ejemplo, si $p>0.5$ etiquetamos los datos como `1`, lo que representa una predicción de que es más probable que un individuo tenga diabetes, si $p< 0.5$ lo etiquetamos como `0` para representar que es más probable que no tenga diabetes.
:::
