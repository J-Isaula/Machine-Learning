...
En este capitulo trabajaremos con un conjunto de datos de música, tanto para problemas de clasificación, como de regresión, donde:
...
## EDA w/ con características categoricas

::::: columns
::: {.column style="font-size:15px"}
Primero construiremos un modelo de regresión, utilizando todas las características del conjunto de datos, para predecir la popularidad de las canciones, existe una característica categórica, el genero con 10 valores posibles.

![](img/g_28.png){fig-align="center" width="500"}

Este diagrama de caja, muestra como varia la popularidad según el genero.
...
# neg_mean_squared_error: devuelve em MSE negativo. Esto se deba a que las métricas
# de Validación cruzada de scikit-learn presuponen que una puntuación más alta es 
# mejor, por lo que el MSE se cambia a negativo para contrarrestarlo. 
...
::::: columns
::: {.column style="font-size:15px"}
Veamos ahora como tratar los datos que faltan. Cuando no hay ningún valor para una característica en una fila concreta, hablamos de datos que falta. Esto puede ocurrir:

-   Porqué no hubo observación.

-   O porqué los datos pueden estar corruptos

Sea cual sea el motivo tenemos que solucionarlo, antes hemos trabajado con el conjunto de datos de música. Continuemos con el mismo.
...
![](img/g_29.png){fig-align="center" width="500"}

Como podemos ver a cada característica le faltan entre 8 a 200 valores. Un enfoque habitual consiste en eliminar las observaciones emitidas que representen menos del 5% de todos los datos.Para ello, utilizamos el método `.dropna()` de pandas.
...
::: {.column style="font-size:15px"}
Volviendo a comprobar el DataFrame, vemos menos valores pérdidos:

![](img/g_30.png){fig-align="center" width="500"}

#### Imputando Valores Pérdidos

Otra opción es, imputar valores pérdidos, esto supone hacer una estimación sobre cuales podrían ser los valores que faltan.

-   Podemos imputar la media de todas las entradas, no omitidas de una característica determinada.

-   También podemos utilizar otros valores como la mediana.

-   Para los valores categóricos, solemos imputar el valor más frecuente. Hay que tener en cuenta que debemos dividir nuestros datos antes de imputarlos, para evitar que se filtre información del conjunto de pruebas a nuestro modelo.

-   Este concepto se conoce como fuga de datos.
:::
:::::
...
A continuación se muestra un flujo de trabajo de imputación para predecir la popularidad de una canción.
...
random_state = 12)
# para imputar los valores categoricos pérdidos, instanciamos: 
imp_cat = SimpleImputer(strategy = "most_frequent")
X_train_cat = imp_cat.fit_transform(X_train_cat)
X_test_cat = imp_cat.transform(X_test_cat)

# Para nuestros datos numericos instanciamos otro imputador
imp_num = SimpleImputer() # Por defecto, rellena los valores con la media 
X_train_num = imp_num.fit_transform(X_train_num)
X_test_num = imp_num.fit_transform(X_test_num)
# Combinamos nuestros datos de entrenamiento 
X_train = np.append(X_train_num, X_train_cat, axis = 1) 
# Repetimos esto, para nuestros datos de prueba 
X_test = np.append(X_test_num, X_test_cat, axis = 1)

```

Debido a su capacidad para transformar nuestros datos a los imputadores se les conoce como transformadores.

#### Imputando con pipeline

También podemos imputar utilizando una canalización, que es un objeto utilizado para ejecutar una serie de transformación y construir un modelo en un único flujo de trabajo.
:::
...
Para construir un pipeline, construimos una lista de pasos que contengan tuplas con los nombres de los pasos especificados:
...
![](img/g_31.png){width="500"}

Debemos tener encuenta que, en un pipeline, todos los pasos excepto el último deben ser transformadores.
:::
:::::
...
::::: columns
::: {.column style="font-size:15px"}
La imputación de datos es uno de los varios pasos de preprocesamiento importantes para el aprendizaje automatico. Acontinuación vamos a abordar otro, `centrar y escalar` nuestros datos. Utilizamos `.describe()` para comprobar los rangos de algunas de nuestras variables de características en el conjunto de datos de música.

```{bash}
print(music_df[["duration_ms", "loudness", "speechiness"]].describe())
```

![](img/g_32.png){width="500"}

Podemos ver que los rangos varian mucho, **duration_ms** ocila entre 0 y 1.62 millones.

-   Muchos modelos de Machine Learning, utilizan algún tipo de distancia para informarce, de modo que si tenemos características a escalas mucho mayores pueden influir de forma desproporcionada en nuestro modelo, por ejemplo,

-   KNN utiliza explicitamente la distancia al hacer predicciones por esta razón lo que queremos es que las características esten en una escala similar.

-   Para conseguirlo, podemos normalizar o estandarizar nuestros datos, lo que a menudo se conoce como **escalado y centrado.**
:::

::: {.column style="font-size:15px"}
#### Como escalar nuestros datos?

Existen varias formas de escalar nuestros datos:

-   Para cada columna podemos restar la media y dividirla por la varianza, de forma que todas las características se centren en 0 y tengan una varianza de 1, esto se llama **normalización**.

-   También, podemos restar el *mínimo* y dividir por el *rango de los datos*, de modo que el conjunto de datos normalizado tenga un mínimo 0 y un máximo 1.\

    -   O bien, podemos centrar los datos para que vayan de -1 a 1.

Aquí, llevaremos a cabo la normalización, sin embargo, Scikit-Learn dispone de funciones para otros tipos de escalado.
...
::::: columns
::: {.column style="font-size:15px"}
#### Comparando performance usando data sin scalar
...
Como podemos observar, escalando nuestros datos mejoramos la precisión.

Veamos también, como podemos utilizar la CV con un pipeline.

#### CV y Escalado en un Pipeline

```{python}
from sklearn.model_selection import GridSearchCV
# Primero construimos la canalización 
steps = [("scaler", StandardScaler()), ("knn", KNeighborsClassifier())]
pipeline = Pipeline(steps)

# Acontinuación especificamos nuestro espacio de hiperparámetros 
...
# Realizamos una busqueda en cuadricula sobre nuestros parámetros 
cv = GridSearchCV(pipeline, param_grid = parameters)

# Tras esto, lo ajustamos a nuestros datos de Train 
cv.fit(X_train, y_train)

# Hacemos predicciones utilizando nuestro conjunto de prueba 
y_pred = cv.predict(X_test)
print(cv.best_score_)
```

vemos que la puntuación es similar que la del modelo con los datos escalados.
...
::: {.column style="font-size:15px"}
Al imprimir nuestros mejores parámetros, el modelo óptimo tiene 6 vecinos.
:::
:::::
...
::::: columns
::: {.column style="font-size:15px"}
Hemos cubierto todas las partes del flujo de trabajo del aprendizaje supervisado, pero como decidimos que modelo utilizar en primer lugar?. Se trata de una pregunta compleja, en la que la respuesta depende de nuestra situación. Sin embargo, existen algunos principios que pueden guiarnos a la hora de tomar una decisión:

-   El tamaño de nuestro conjunto de datos, es un factor a considerar,\
    -   menos características implica un modelo más sencillo.\
    -   esto puede reducir el tiempo de entrenamiento.\
    -   además, algunos modelos como las redes neuronales artificiales requieren una gran cantidad de datos para funcionar bien.
-   Interpretabilidad\
    -   Puede que necesitemos un modelo interpretable, para poder explicar a las partes interesadas como se hicieron las predicciones.\
    -   Podemos tomar como ejemplo la Regresión Lineal, con la que podemos calcular e interpretar los coeficientes del modelo.
-   Flexibilidad\
    -   Puede ser importante para obtener predicciones lo más precisas posibles. Por lo general, los modelos flexibles hacen menos supociones sobre los datos, por ejemplo,\
    -   Un modelo KNN no supone una relación lineal entre las características y el objetivo.

Ten encuenta que Scikit-Learn, permite utilizar los mismos métodos para la mayoría de los modelos, esto facilita su comparación.
:::
...
-   Rendimiento del modelo de regresión:\
    -   RMSE\
    -   R-squared
-   Rendimiento modelos de clasificación:\
    -   Accuracy\
    -   Confusion matrix\
    -   Precisión, recall, F1-score\
    -   ROC AUC
-   Varios modelos y evaluar el rendimiento desde el primer momento

#### Una nota sobre la escalabilidad

Recordemos que el rendimiento de algunos modelos son afectados por la scalabilidad
...
Así que, por normal general, es mejor escalar nuestros datos antes de evaluar los modelos. Evaluaremos tres modelos para la clasificación binaria del genero de las canciones:
...
# Creamos un diccionario con los nombres de nuestros modelos 
...
# Recorremos en bucle los modelos de nuestro diccionario 
...
::: {.column style="font-size:15px"}
```{python}
# Creamos un diagrama de caja de nuestros resultados 
